...
### 세미나에서 여러 조원이 함께 쓰기 (권장)
1. 이 저장소를 **GitHub Pages**로 배포 → 조원 전원이 같은 URL로 접속
   - GitHub 저장소 → Settings → Pages → Branch를 `claude/eloquent-rubin-iTxUl`(또는 main) `/root`로 지정
...
> *(실시간 동시편집 동기화는 백엔드가 필요해 현재는 '각자 입력 → JSON 병합' 방식입니다. 데이터 모델은 추후 실시간 연동까지 확장 가능하도록 설계됨)*
